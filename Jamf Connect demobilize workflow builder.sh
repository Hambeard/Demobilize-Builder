...
if [[ "$resultHooksCode" == "201" ]]; then
	...
	echo "Error: Error enabling Login hooks: $resultHooksCode"
fi
...
if [[ "$resultCheckLoginCode" == "201" ]]; then
	...
	echo "Error: Error enabling Check for policies at login: $resultCheckLoginCode"
fi
...
if [[ "$resultCatCode" == "201" ]]; then
	...
	echo "Error: Error creating Jamf Connect category: $resultCatCode"
fi
...
if [[ "$resultEACode" == "201" ]]; then
	...
	echo "Error: Error creating Extension Attribute: $resultEACode"
fi
...
<script>
	...
	if [[ "$resultScriptCode" == "201" ]]; then
		...
		echo "Error: Error creating Demobilize - Helper script: $resultScriptCode"
	fi
fi
...
resultScript2Code=$( echo "$resultScript2" | awk -F"##" '{ print $2 }' )
	if [[ "$resultScript2Code" == "201" ]]; then
		...
		echo "Error: Error creating Demobilize - Trigger script: $resultScript2Code"
	fi
fi
...
<script>
	...
	if [[ "$resultScript3Code" == "201" ]]; then
		...
		echo "Error: Error creating Demobilize - Migrate script: $resultScript3Code"
	fi
fi
...
if [[ "$resultSG1Code" == "201" ]]; then
	...
	echo "Error: Error creating Smart Group (Demobilize - No Mobile Accounts): $resultSG1Code"
fi
...
if [[ "$resultSG2Code" == "201" ]]; then
	...
	echo "Error: Error creating Smart Group (Demobilize - Mobile Accounts): $resultSG2Code"
fi
...
if [[ "$resultSG3Code" == "201" ]]; then
	...
	echo "Error: Error creating Smart Group (Demobilize - Jamf Connect not installed): $resultSG3Code"
fi
...
if [[ "$resultSG4Code" == "201" ]]; then
	...
	echo "Error: Error creating Smart Group (Demobilize - Jamf Connect installed): $resultSG4Code"
fi
...
if [[ "$resultPPPCCode" == "201" ]]; then
	...
	echo "Error: Error creating Demobilize - Jamf System Events PPPC configuration profile: $resultPPPCCode"
fi
